...
def ejecucion(codmes):
    ...
    ruta_ldng_var_metereologicas = f"{bucket_name}/datalake/landing/var_metereologicas/cloud_cover/"
...
def transformacion(ruta,ruta_dist,codmes):
    ...
    df2 = df1filter.withColumn("latitud",col("lat").cast(FloatType()))\
        ...
        .withColumn("valor_var",col("Cloud_Cover_Mean").cast(FloatType()))
    ...
    df4 = df3.drop('lat','lon','time','Cloud_Cover_Mean')
    
    df5 = df4.withColumn("codigovar", lit("CUDNU"))
