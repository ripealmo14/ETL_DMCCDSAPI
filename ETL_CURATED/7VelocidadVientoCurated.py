...
def ejecucion(codmes):
    ...
    ruta_ldng_var_metereologicas = f"{bucket_name}/datalake/landing/var_metereologicas/velocidad_viento/"
...
def transformacion(ruta,ruta_dist,codmes):
    ...
    df2 = df1filter.withColumn("latitud",col("lat").cast(FloatType()))\
        ...
        .withColumn("valor_var",col("Wind_Speed_10m_Mean").cast(FloatType()))
    ...
    df4 = df3.drop('lat','lon','time','Wind_Speed_10m_Mean')
    
    df5 = df4.withColumn("codigovar", lit("VEDVI"))
