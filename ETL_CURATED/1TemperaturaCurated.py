...
def ejecucion(codmes):
    ...
    ruta_ldng_var_metereologicas = f"{bucket_name}/datalake/landing/var_metereologicas/2m_temperature/"
...
def transformacion(ruta,ruta_dist,codmes):
    ...
    df2 = df1filter.withColumn("latitud",col("lat").cast(FloatType()))\
        ...
        .withColumn("valor_var",col("Temperature_Air_2m_Mean_24h").cast(FloatType()))
    ...
    df4 = df3.drop('lat','lon','time','Temperature_Air_2m_Mean_24h')
    
    df5 = df4.withColumn("codigovar", lit("TEMPR"))
